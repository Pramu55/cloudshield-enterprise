...
import { BarChart3, FileCheck2, ShieldCheck, WalletCards, Info, Server, ShieldAlert, AlertTriangle, CheckCircle2, FileText, RefreshCw, Layers, Gauge, GitPullRequestDraft, SearchCheck, ClipboardList, Building2, Network, Bot, Play } from "lucide-react";
...
export default function DashboardHome() {
  ...
  const { data: automationLatest } = useCloudShieldData<AutomationLatestResponse>(
    "/api/v1/automation/latest",
    {
      ...
      readiness: { mode: "EVALUATION", blockedReason: "AWS execution disabled. Assessment uses CloudShield DB/sample records." },
      awsApiCallExecuted: false,
      scannerRun: false
    }
  );
  ...
  return (
    ...
    >
      ...
      <section className="mb-6 grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <InsightPanel
          title="CloudShield Intelligence Engine"
          description="Latest automated assessment status and generated executive summary."
        >
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px]">
            <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4">
              <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-indigo-700">
                <Bot size={14} />
                AI-assisted deterministic assessment
              </div>
              <p className="text-sm leading-6 text-slate-700">
                {automationLatest.intelligenceSummary?.executiveSummary ??
                  "No automated assessment has been generated yet. Start one to produce risk prioritization, evidence summary, remediation drafts, and an internal report preview."}
              </p>
            </div>
            <div className="rounded-xl border border-line bg-white p-4">
              <StatusMatrix
                items={[
                  { label: "Status", value: automationLatest.assessment?.status ?? "not started", tone: automationLatest.assessment ? "good" : "warning" },
                  { label: "Mode", value: automationLatest.readiness.mode, tone: "warning" },
                  { label: "AWS API", value: automationLatest.assessment?.safetyFlags.awsApiCallExecuted ?? automationLatest.awsApiCallExecuted, tone: "good" },
                  { label: "Scanner", value: automationLatest.assessment?.safetyFlags.scannerRun ?? automationLatest.scannerRun, tone: "good" }
                ]}
              />
              <Link
                className="cs-action-primary mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-bold"
        ...
        <InsightPanel
          title="Intelligent next actions"
          description="Generated by the deterministic engine after the latest assessment."
        >
          <div className="space-y-3">
            {(automationLatest.intelligenceSummary?.nextActions?.length ? automationLatest.intelligenceSummary.nextActions.slice(0, 4) : [
              { label: "Run automated assessment", owner: "Platform", priority: "P0", detail: "Generate the first intelligence summary from existing CloudShield records." }
            ]).map((action, index) => (
              <div className="rounded-xl border border-line bg-white p-3" key={`${String(action.label)}-${index}`}>
                <p className="text-sm font-bold text-ink">{String(action.label)}</p>
                <p className="mt-1 text-xs leading-5 text-slate-500">
                  {String(action.owner)} / {String(action.priority)} / {String(action.detail)}
                </p>
              </div>
            ))}
          </div>
