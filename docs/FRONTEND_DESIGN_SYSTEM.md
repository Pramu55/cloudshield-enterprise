...
Monitoring run list and detail use the same explicit projection trust boundary. The shared projection drops `organizationId`, arbitrarily nested raw errors, provider request IDs, unknown fields, and invalid counts. A conditionally safe error summary allows only valid, length-bounded message and category strings, plus a boolean retryable flag, without introducing undefined properties.
