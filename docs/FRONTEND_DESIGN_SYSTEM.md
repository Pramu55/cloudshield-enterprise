...
Monitoring alert detail uses the same trust boundary. The frontend projects only approved alert fields and drops `mappedEvidence` because the shared contract exposes it as arbitrary records. A valid zero-evidence alert uses neutral copy and never implies safety. Lifecycle rendering is cross-field validated: unresolved states cannot carry `resolvedAt`, while `RESOLVED` requires it. Acknowledge and resolve controls do not optimistically change status; a validated read must confirm the new lifecycle. Until shared mutation response schemas exist, mutation bodies are not treated as authority and no automatic retry is permitted.
