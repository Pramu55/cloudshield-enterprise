...
Acknowledgement and resolution now share one strict authoritative acceptance envelope: HTTP 200 `{ status: "ok" }`. Acceptance is not lifecycle confirmation and does not contain alert status, alert data, `resolvedAt`, audit metadata, or arbitrary records. The detail page confirms lifecycle only through one validated detail read; the list confirms only through one validated alerts-list refresh. The list refresh must successfully complete before lifecycle is treated as confirmed; valid mutation acceptance without a successful validated refresh leaves the lifecycle unconfirmed. Malformed non-empty 2xx mutation response becomes `CONTRACT_INVALID`, skips the confirmation refresh, and cannot produce success. A malformed refresh response also becomes `CONTRACT_INVALID`. A blank HTTP 200 is not reinterpreted as the acceptance envelope. Generic 204/205 and successful blank-body handling remains unchanged in the central client, but these monitoring routes currently return non-empty JSON. Mutations execute once, no retry or optimistic lifecycle change occurs.
...
Remaining monitoring gaps: monitoring evaluate response, monitoring run-detail response, safe evidence summary/history contracts, and explicit permission/capability authority for monitoring mutations remain outside this slice. The monitoring platform is still not complete.
